...
const pagoSchema = z.object({
  cuentaCobroId: z.preprocess((v) => Number(v), z.number().int().positive("Cuenta de cobro no válida")),
  // RN-023: el valor ordenado debe ser positivo.
  valorPagado: z.preprocess((v) => Number(v), z.number().positive("RN-023: el valor del pago debe ser mayor a cero")),
  comprobante: z.preprocess((v) => (v === "" || v == null ? undefined : v), z.string().trim().max(120).optional()),
  medioPago: z.preprocess(
    (v) => (v === "" || v == null ? undefined : v),
    z.enum(["Orden bancaria", "Cheque", "Giro"]).optional()
  ),
});
...
function readPagoForm(fd: FormData) {
  return {
    cuentaCobroId: fd.get("cuentaCobroId"),
    valorPagado: fd.get("valorPagado"),
    comprobante: fd.get("comprobante"),
...
export async function crearCuenta(_prev: FormState, fd: FormData): Promise<FormState> {
  ...
  if (d.informeId != null) {
    ...
    if (informe.contratoId !== d.contratoId)
      return { fieldErrors: { informeId: "RN-006: el informe debe pertenecer al mismo contrato de la cuenta." } };
  }
...
export async function aprobarCuenta(fd: FormData): Promise<void> {
  ...
  const id = Number(fd.get("id"));
  const cuentaPrevia = await prisma.cuentaCobro.findUnique({ where: { id } });
  if (!cuentaPrevia || cuentaPrevia.estado !== "Registrada") {
    revalidatePath(`/financiera/${id}`);
    redirect(`/financiera/${id}`);
  }
  if (cuentaPrevia.createdById && cuentaPrevia.createdById === Number(session.user.id))
    throw new Error("Segregación de funciones (RN-025): no puedes aprobar lo que tú registraste.");

  await prisma.$transaction(async (tx) => {
    // Re-leer dentro de la transacción para evitar TOCTOU con aprobaciones concurrentes.
    const cuenta = await tx.cuentaCobro.findUnique({ where: { id } });
    if (!cuenta || cuenta.estado !== "Registrada")
      throw new Error("La cuenta de cobro ya no está pendiente de aprobación.");

    // RN-018: si hay pólizas Aprobadas del contrato y alguna está vencida, bloquea la aprobación.
    const polizas = await tx.poliza.findMany({ where: { contratoId: cuenta.contratoId, estado: "Aprobada" } });
    const hoy = new Date();
    ...
    const rubro = await tx.rubro.findUnique({ where: { id: cuenta.rubroId } });
    if (!rubro) throw new Error("El rubro de esta cuenta no existe.");
    const cuentasComprometidas = await tx.cuentaCobro.findMany({
      where: { rubroId: rubro.id, estado: { in: ["Aprobada", "Pagada"] }, id: { not: cuenta.id } },
      select: { valorAprobado: true, valorCobrado: true },
    });
    const comprometido = cuentasComprometidas.reduce((acc, c) => acc + (c.valorAprobado ?? c.valorCobrado ?? 0), 0);
...
export async function ordenarPago(fd: FormData): Promise<void> {
  ...
  const parsed = pagoSchema.safeParse(readPagoForm(fd));
  if (!parsed.success) throw new Error(parsed.error.issues[0]?.message ?? "Datos de pago no válidos.");
  const { cuentaCobroId, valorPagado, comprobante, medioPago } = parsed.data;

  const pago = await prisma.$transaction(async (tx) => {
    // Toda la lectura de saldos + la creación de la orden quedan en la misma transacción para
    // evitar que dos órdenes concurrentes lean el mismo saldo disponible antes de que la otra escriba.
    const cuenta = await tx.cuentaCobro.findUnique({
    ...
    const pagosRubro = await tx.pago.findMany({ where: { cuentaCobro: { rubroId: cuenta.rubroId }, estado: "Aprobado" } });
...
export async function aprobarPago(fd: FormData): Promise<void> {
  ...
  const id = Number(fd.get("id"));
  const pagoPrevio = await prisma.pago.findUnique({ where: { id } });
  if (!pagoPrevio || pagoPrevio.estado !== "Ordenado") {
    revalidatePath(`/financiera/${pagoPrevio?.cuentaCobroId ?? ""}`);
    redirect(`/financiera/${pagoPrevio?.cuentaCobroId ?? ""}`);
  }
  if (pagoPrevio.createdById && pagoPrevio.createdById === Number(session.user.id))
    throw new Error("Segregación de funciones (RN-025): no puedes aprobar una orden de pago que tú mismo ordenaste.");

  const cuentaId = await prisma.$transaction(async (tx) => {
    // Re-leer todo dentro de la transacción para evitar TOCTOU con aprobaciones concurrentes.
    const pago = await tx.pago.findUnique({
      where: { id },
      include: { cuentaCobro: { include: { rubro: true, pagos: { where: { estado: "Aprobado" } } } } },
    });
    if (!pago || pago.estado !== "Ordenado") throw new Error("La orden de pago ya no está pendiente de aprobación.");
    ...
    if (sumaAprobadosCuenta + pago.valorPagado >= (cuenta.valorAprobado ?? 0)) {
      await tx.cuentaCobro.update({ where: { id: cuenta.id }, data: { estado: "Pagada" } });
    }
...
export async function rechazarPago(fd: FormData): Promise<void> {
  ...
  if (pago.createdById && pago.createdById === Number(session.user.id))
    throw new Error("Segregación de funciones (RN-025): no puedes rechazar una orden de pago que tú mismo ordenaste.");
